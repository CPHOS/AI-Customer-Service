...
class AddNewSchoolByName(CustomOperation):
    ...
    def execute(self, cursor: pymysql.cursors.Cursor):
        try:
            ...
            cursor.execute(
                "select * from cmf_tp_school where `school_name` = '{}' and `area` = {}".format(self.Name, self.AreaId)
            )
            result = cursor.fetchall()
            if len(result) > 0:
                ...
                return result[0][0]
            cursor.execute(
                "insert into cmf_tp_school (`school_name`,`area`) values ('{}',{})".format(self.Name, self.AreaId)
            )
            cursor.execute(
                "select * from cmf_tp_school where `school_name` = '{}' and `area` = {}".format(self.Name, self.AreaId)
            )
