    ...
    def execute(self, cursor: pymysql.cursors.Cursor):
        ...
        if result[0][-3] == 1:
            ...
            if len(exam_id) >= 1:
                ...
                exam_id = exam_id[0][0]
                cursor.execute(
                    "select count(*) from cmf_tp_correct as a "
                    "join cmf_tp_subject as b on a.p_id = b.id "
                    "join cmf_tp_test_paper as c on b.p_id = c.id "
                    "where c.p_id = {} and a.user_id = {} and a.status = 1".format(exam_id, self.ArbiterId)
                )
        ...
        if result[0][-3] == 1:
            ...
            if len(exam_id) >= 1:
                ...
                cursor.execute(
                    ...
                    "join cmf_tp_subject as b on a.p_id = b.id "
                    "join cmf_tp_test_paper as c on b.p_id = c.id "
                    "where c.p_id = {} and a.user_id = {} and a.status = 1".format(exam_id, self.SubCoachId)
                )
                returned = cursor.fetchall()
                if returned[0][0] > 0:
                    raise Exception("The SubCoach has not finished correcting the test papers!")
...
class ChangeAllTypesMarkingSubject(CustomOperation):
    def __init__(self, ChangedUserId: int, ItsNewMarkingSubject: int):
        ...
        assert self.NewMarkingSubject in range(1, 11), "Marking subject must be in [1, 10]"
        ...
        if result[0][-3] == 1:
            ...
            if len(exam_id) >= 1:
                ...
                cursor.execute(
                    ...
                    "join cmf_tp_subject as b on a.p_id = b.id "
                    "join cmf_tp_test_paper as c on b.p_id = c.id "
                    "where c.p_id = {} and a.user_id = {} and a.status = 1".format(exam_id, self.Id)
                )
                returned = cursor.fetchall()
                if returned[0][0] > 0:
                    raise Exception("The SubCoach has not finished correcting the test papers!")
...
class ChangeAllTypesUploadLimit(CustomOperation):
    def __init__(self, ChangedUserId: int, NewUploadLimit: int):
        ...
        assert 0 <= NewUploadLimit <= 60000, 'UploadLimit must be non-negative and <= 60000!'
...
class VerifyTeacherUserToBeSupTeacher(CustomOperation):
    ...
    def execute(self, cursor: pymysql.cursors.Cursor):
        ...
        if result[0][-4] == 1:
            ...
            cursor.execute(
                "update cmf_tp_member set `user_name` = '{}', `school_id` = {}, `subject`={}, `limit`={}, `status`=1 "
                "where id = {}".format(self.Name, self.SchoolId, self.ViewProblem, self.UploadLimit, self.Id)
            )
            cursor.fetchall()
